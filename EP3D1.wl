...
Print["Paradox R to L rate is ",N[(xa-(1-x3))/(2*x3)]];
SeedRandom[0];
...
T3=NSolve[{(circ/.{x->pt[[1]],y->pt[[2]],z->pt[[3]]})==1,t>10^-8,pt[[1]]>xa},t,Reals];
...
If[picNum1<=Sumpic,pict=Show[ContourPlot3D[{eli1==1},{x,-u,xa},{y,-u,u},{z,-u,u},BoundaryStyle->None,ContourStyle->None],ContourPlot3D[{circ==1},{x,xa,u},{y,-u,u},{z,-u,u},BoundaryStyle->None,ContourStyle->None],ContourPlot3D[{c1==1,c2==1},{x,-u,u},{y,-u,u},{z,-u,u},ColorFunction->"DarkRainbow",BoundaryStyle->None,Mesh->None],Graphics3D[{Red,Thick,Line[P]}],PlotRange->All,BoxRatios ->Automatic];If[picNum1<10,Export[StringTake[figpath,StringLength[figpath]-4]<>"L0"<>ToString[picNum1++]<>".png",pict],Export[StringTake[figpath,StringLength[figpath]-4]<>"L"<>ToString[picNum1++]<>".png",pict]]];
]]
...
If[picNum2<=Sumpic,pict=Show[ContourPlot3D[{eli1==1},{x,-u,xa},{y,-u,u},{z,-u,u},BoundaryStyle->None,ContourStyle->None],ContourPlot3D[{circ==1},{x,xa,u},{y,-u,u},{z,-u,u},BoundaryStyle->None,ContourStyle->None],ContourPlot3D[{c1==1,c2==1},{x,-u,u},{y,-u,u},{z,-u,u},ColorFunction->"DarkRainbow",BoundaryStyle->None,Mesh->None],Graphics3D[{Red,Thick,Line[P]}],PlotRange->All,BoxRatios ->Automatic];If[picNum2<10,Export[StringTake[figpath,StringLength[figpath]-4]<>"R0"<>ToString[picNum2++]<>".png",pict],Export[StringTake[figpath,StringLength[figpath]-4]<>"R"<>ToString[picNum2++]<>".png",pict]]];
]]
